...
const MeetingsModule: React.FC<MeetingsModuleProps> = ({ meetings, loading, viewType = 'home' }) => {
  ...
  const monthData = useMemo(() => {
    ...
    // Adjust for RTL/Friday start if needed, but standard 0-6 is fine for grid
    const days = [];
  ...
  const meetingsForSelectedDay = useMemo(() => {
    return meetings.filter(m => 
      m.startTime.getDate() === selectedDay.getDate() &&
      m.startTime.getMonth() === selectedDay.getMonth() &&
      m.startTime.getFullYear() === selectedDay.getFullYear()
    ).sort((a, b) => a.startTime.getTime() - b.startTime.getTime());
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[11px] font-black text-emerald">
                      {m.startTime.toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' })}
                    </span>
                  ...
                  <p className="text-[10px] text-gray-400 font-medium">{m.platform} • {m.attendees} مشارك</p>
                </div>
                {m.meetingLink && (
                  <button 
        ...
        <div className="grid grid-cols-7 gap-1 mb-2">
          ...
          {monthData.map((date, idx) => {
            ...
            const hasMeeting = meetings.some(m => 
              m.startTime.getDate() === date.getDate() && 
              m.startTime.getMonth() === date.getMonth() &&
              m.startTime.getFullYear() === date.getFullYear()
            );
            ...
            return (
              <button
                ...
                className={`relative h-11 flex flex-col items-center justify-center rounded-xl transition-all ${
                  isSelected ? 'bg-emerald text-white shadow-lg shadow-emerald/20' : isToday ? 'bg-emerald/10 text-emerald' : 'bg-white text-gray-600'
                }`}
              >
                ...
                <div className="flex justify-between items-start mb-3">
                  <div className="flex flex-col">
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      <span className="text-[11px] font-black text-emerald">{m.startTime.toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' })}</span>
